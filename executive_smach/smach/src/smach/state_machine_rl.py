...
class StateMachineRL(smach.container.Container):
	...
	def add(label, state, transitions = None, remapping={}):
		...
		self._label_to_outcomes[label] = state._outcomes
		self._transitions[label] = transitions
		#if type(state) == smach.StateMachineRL:
	#		self.add_learner(learner, label)
		#self.add_learner(self.state_label, label)
		return state
	...
	def get_internal_edges(self):
		...
		for (container_outcome, outcomes) in ((k,self._outcome_map[k]) for k in self._outcome_map):
			for label in self._labels:
				allOutcomes = list()
				for ele in self._states[label]._outcomes:
					allOutcomes.append(ele)
					for (key,value) in self._outcome_map.iteritems():
						if ele in value:
							allOutcomes.append(key)
				if container_outcome not in allOutcomes:
					continue
				if type(self._states[label]) != smach.StateMachineRL:
					int_edges.append((outcomes, label, container_outcome))
